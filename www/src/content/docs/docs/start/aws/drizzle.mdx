...
Your can apply migrations automatically by invoking a lambda function linked to your database.
...
Now define the handler in `src/migrator.ts`
...
Passing the time as input forces the migrator function to run on each deployment.
